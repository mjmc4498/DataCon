...
*   **Preguntas y Respuestas (RAG-lite)**: Un sistema "RAG-lite" que utiliza los resultados de la búsqueda para responder preguntas con citas directas del contenido de las páginas.
...
*   **Interfaz Moderna**: Tema oscuro y diseño responsivo construido con Bootstrap 5.
...
Debido a las restricciones de seguridad de los navegadores (CORS), las solicitudes directas a la API de Confluence desde un archivo local (`file://`) o un dominio diferente probablemente fallarán. Para solucionar esto, el proyecto incluye un **servidor proxy CORS**.
...
*   **Enfocar Búsqueda**: Presione la tecla `/` en cualquier momento para enfocar el cuadro de búsqueda principal.
*   **Búsqueda Simple**: Escriba texto y presione Enter. La herramienta buscará páginas que contengan su texto en los espacios por defecto.
*   **Búsqueda CQL**: Escriba una consulta CQL válida para una búsqueda más precisa.
*   **Preguntas**: Vaya a la sección "Pregúntale a tu Confluence", escriba una pregunta y la herramienta intentará encontrar fragmentos relevantes en las páginas para construir una respuesta.
